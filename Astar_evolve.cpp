...
void AddCloseList(AstarList *OpenList, AstarList *CloseList, int x, int y, int position[MaxSize][MaxSize], int direction[4][2], int map[MaxSize][MaxSize], Path path[MaxSize], int *sum, int *isOk);
void GetF(AstarList *OpenList, Aelem *elem, int x, int y);
int Astar_Find_road(int starx, int starty, int x, int y, Path path[MaxSize], int map[MaxSize][MaxSize]);
...
/*int main()    用法示例
{
	...
	Path path[500];
	int a = 0;
	a=Astar_Find_road(1, 1, 5, 5, path, map);
	printf("%d", a);
}*/
