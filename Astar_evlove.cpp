...
int Astar_Find_road(int startx, int starty, int x, int y, Path path[MaxSize], int map[MaxSize][MaxSize])//ʹ��Astar�㷨ʱ���õĽӿ�
{
	...
	InitList(&CloseList);
	Aelem elem;
	elem.x = startx, elem.y = starty;//elem��Ҫ��ʼ��Ҫ��ֵΪ��ͷ��ʼ��λ��
	elem.parent = -1;
...
void AddCloseList(AstarList *OpenList, AstarList *CloseList, int x, int y, int position[MaxSize][MaxSize], int direction[4][2], int map[MaxSize][MaxSize], Path path[MaxSize], int *sum, int *isOk)//����x��y��ʾҪ�������յ�������
{
	int Fmin = INF;
	int j;
	Aelem e;
